package org.test.itv.checkout.service;

import org.test.itv.checkout.datasource.DataSourceAdapter;
import org.test.itv.checkout.dto.Entity;
import org.test.itv.checkout.dto.UpdateTask;
import org.test.itv.checkout.types.OperationType;
import org.test.itv.checkout.util.KEYS;

import java.util.List;

/**
 * Created on 26/11/18.
 */
public class UpdateOperation<E extends Entity> extends BaseOperation<E, UpdateTask<E>> {

    public UpdateOperation() {
        super(OperationType.UPDATE);
    }


    public UpdateOperation(Class<E> entityClass) {
        super(entityClass, OperationType.UPDATE);
    }

    @Override
    public void process(UpdateTask<E> task) {
        try {
            DataSourceAdapter resource = dataProvider.getDataSource(KEYS.APP_NAME);
            List<E> result = resource.update(task.getEntities(), task.getEntityClass());
            task.setEntities(result);
        } catch (Exception e) {
            String message = "ERROR during the Update Operation for the Task ".concat(task.getId());
            logger.error(message, e);
            task.putError(message);
        }
    }
}
